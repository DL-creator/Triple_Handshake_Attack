...
type FlexClientKeyExchange =
    ...
    static member receiveECDHE (st:state, fske:FServerKeyExchange, ?nsc:nextSecurityContext) : state * nextSecurityContext * FClientKeyExchange =
        let nsc = defaultArg nsc FlexConstants.nullNextSecurityContext in
        let epk = {nsc.secrets with kex = fske.kex} in
        let nsc = {nsc with secrets = epk} in
    ...
    /// <param name="nsc"> Next security context being negotiated and containing the key exchange mechanism retreived from a previous server key exchange</param>
    /// <returns> Updated state * Next security context * FClientKeyExchange message record </returns>
    static member receiveECDHE (st:state, nsc:nextSecurityContext) : state * nextSecurityContext * FClientKeyExchange =
        let kexecdh =
            match nsc.secrets.kex with
    ...
    /// <returns> Updated state * FClientKeyExchange message record </returns>
    static member receiveECDHE (st:state, kexecdh:kexECDH) : state * FClientKeyExchange =
        LogManager.GetLogger("file").Info("# CLIENT KEY EXCHANGE : FlexClientKeyExchange.receiveECDHE");
        ...
        | HT_client_key_exchange  ->
            ...
            | Correct(ecp_y) ->
                ...
                LogManager.GetLogger("file").Debug(sprintf "--- Public ECPoint : %s ; %s" (Bytes.hexString(ecp_y.ecx)) (Bytes.hexString(ecp_y.ecy)));
                LogManager.GetLogger("file").Debug(sprintf "--- Payload : %s" (Bytes.hexString(payload)));
                st,fcke
            )
        | _ -> failwith (perror __SOURCE_FILE__ __LINE__ (sprintf "Unexpected handshake type: %A" hstype))
    ...
    /// <returns> FClientKeyExchange message bytes * FClientKeyExchange record * </returns>
    static member prepareECDHE (kexecdh:kexECDH) : FClientKeyExchange * kexECDH =
        let parameters = CommonDH.DHP_EC(ECGroup.getParams kexecdh.curve) in
        // We override the public and secret exponent if one of the two is empty
    ...
    static member sendECDHE (st:state, fske:FServerKeyExchange, ?nsc:nextSecurityContext, ?fp:fragmentationPolicy) : state * nextSecurityContext * FClientKeyExchange =
        let fp = defaultArg fp FlexConstants.defaultFragmentationPolicy in
        let nsc = defaultArg nsc FlexConstants.nullNextSecurityContext in
        let epk = {nsc.secrets with kex = fske.kex} in
        let nsc = {nsc with secrets = epk} in
    ...
    /// <param name="fp"> Optional fragmentation policy at the record level </param>
    /// <returns> Updated state * Next security context * FClientKeyExchange message record </returns>
    static member sendECDHE (st:state, nsc:nextSecurityContext, ?fp:fragmentationPolicy) : state * nextSecurityContext * FClientKeyExchange =
        let fp = defaultArg fp FlexConstants.defaultFragmentationPolicy in
        let kexecdh =
            match nsc.secrets.kex with
            | ECDH(kexecdh) -> kexecdh
    ...
    /// <param name="fp"> Optional fragmentation policy at the record level </param>
    /// <returns> Updated state * FClientKeyExchange message record </returns>
    static member sendECDHE (st:state, kexecdh:kexECDH, ?fp:fragmentationPolicy) : state * FClientKeyExchange =
        LogManager.GetLogger("file").Info("# CLIENT KEY EXCHANGE : FlexClientKeyExchange.sendECDHE");
        let fp = defaultArg fp FlexConstants.defaultFragmentationPolicy in
        ...
        LogManager.GetLogger("file").Debug(sprintf "--- SECRET Value : %s" (Bytes.hexString(ecdh.x)));
        LogManager.GetLogger("file").Debug(sprintf "--- Public Point : %s ; %s" (Bytes.hexString(ecx)) (Bytes.hexString(ecy)));
        st,fcke
