...
let load (path: string) : RSAKey.sk =
    use reader = new StreamReader(path) in 
    let pemReader = new PemReader(reader) in 
    match pemReader.ReadObject() with
    | :? RsaPrivateCrtKeyParameters as rsa ->
        let modulus = abytes (rsa.Modulus.ToByteArrayUnsigned()) in 
        let d = abytes (rsa.Exponent.ToByteArrayUnsigned()) in

        let csk = CoreACiphers.RSASKey(modulus, d) in
        { RSAKey.sk = csk }
    | _ ->
        failwithf "Unsupported key format in %s" path
